...
const LoaderOverlay = ({ onLoaded }) => {
  ...
  // Initialize refs for counter items on counterData length change
  useEffect(() => {
    if (countItemRefs.current.length !== counterData.length) {
  ...
  if (!gsap) return <div>GSAP not available in LoaderOverlay</div>;
...
const SpinnerElement = styled.div`
  width: 50px;
  height: 50px;
  border: 1.4px solid ${({ theme }) => theme.colors.text};
