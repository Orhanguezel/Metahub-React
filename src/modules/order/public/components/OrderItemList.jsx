...
const OrderItemList = ({ items, lang: customLang }) => {
  ...
  return (
    <Items>
      {items.map((item, idx) => (
        <Item key={item.product?._id || item.product || idx}>
          <Left>
            <ProductName>
              {item.product?.name
                ? getLocalized(item.product.name, lang)
                : item.name}
            </ProductName>
          ...
          <Right>
            <Qty>
              {item.quantity} ×{" "}
              <Price>
                {(item.priceAtAddition || item.product?.price)?.toFixed(2) ||
                  "0.00"}
              </Price>
